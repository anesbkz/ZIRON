...
exports.normalizeProductCode = normalizeProductCode;
...
exports.initializeBootstrapGovernance = functions.https.onCall(async (_data, context) => {
    ...
    const targetBootstrapEmail = getBootstrapSuperAdminEmail();
    if (!targetBootstrapEmail) {
        throw new functions.https.HttpsError('failed-precondition', 'System bootstrap configuration missing. BOOTSTRAP_SUPERADMIN_EMAIL environment variable is not configured.');
    }
 ...
 */
exports.CANONICAL_CATALOG_SKUS = {
...
exports.CROCKFORD_ALPHABET = '23456789ABCDEFGHJKMNPQRSTVWXYZ';
/**
 ...
 */
function generateCrockfordSegment(length) {
    const bytes = crypto.randomBytes(length);
    let result = '';
    for (let i = 0; i < length; i++) {
        result += exports.CROCKFORD_ALPHABET[bytes[i] % exports.CROCKFORD_ALPHABET.length];
    }
    return result;
}
/**
 ...
 */
function generateSecureProductCode(phasePrefix) {
 ...
 */
async function assertCanManageCodes(context) {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Caller must be authenticated.');
    }
    const callerUid = context.auth.uid;
    ...
    if (!isAuthorized) {
        throw new functions.https.HttpsError('permission-denied', 'Caller lacks authority to manage product codes (requires SUPER_ADMIN, ADMIN, or PRODUCT_MANAGER).');
    }
...
exports.activateContainerCode = functions.https.onCall(async (data, context) => {
    ...
    return await db.runTransaction(async (transaction) => {
        ...
        if (codeData.status === 'ACTIVATED' || codeData.isActivated) {
            throw new functions.https.HttpsError('already-exists', 'This container has already been activated.');
        }
        if (codeData.status === 'DISABLED' || codeData.status === 'REVOKED') {
            throw new functions.https.HttpsError('failed-precondition', `This container code is ${codeData.status.toLowerCase()} and cannot be activated.`);
        }
        // Read parent batch using batchId if present
        let batchDocRef = null;
        if (codeData.batchId) {
            ...
            if (batchSnap.exists) {
                ...
                if (batchData.status === 'DISABLED') {
                    throw new functions.https.HttpsError('failed-precondition', 'This manufacturing batch is currently on hold or disabled.');
                }
                if (batchData.status === 'ARCHIVED') {
                    throw new functions.https.HttpsError('failed-precondition', 'This manufacturing batch has been archived.');
                }
            }
        }
        ...
        const distinctContainerCodes = new Set();
        ...
        const entitlementsToGrant = ['COMMUNITY_ACCESS', 'PHASE_TRAJECTORY'];
 ...
 */
exports.generateProductCodes = functions.https.onCall(async (data, context) => {
    ...
    if (typeof quantity !== 'number' || !Number.isInteger(quantity) || quantity < 1 || quantity > 500) {
        throw new functions.https.HttpsError('invalid-argument', 'Quantity must be an integer between 1 and 500.');
    }
    // 2. Validate productSku
    if (!productSku || typeof productSku !== 'string' || !exports.CANONICAL_CATALOG_SKUS[productSku]) {
        throw new functions.https.HttpsError('invalid-argument', `Product SKU "${productSku}" is not recognized in the canonical catalog. Allowed: ${Object.keys(exports.CANONICAL_CATALOG_SKUS).join(', ')}`);
    }
    ...
    if (batchData.status !== 'ACTIVE') {
        throw new functions.https.HttpsError('failed-precondition', `Batch "${batchId}" is not ACTIVE (current status: ${batchData.status}). Codes can only be generated for ACTIVE batches.`);
    }
    const skuConfig = exports.CANONICAL_CATALOG_SKUS[productSku];
 ...
 */
exports.createProductBatch = functions.https.onCall(async (data, context) => {
    const { callerUid, callerEmail, callerRoles } = await assertCanManageCodes(context);
    const { productSku, batchNumber: rawBatchNumber, manufactureDate, expiryDate, notes, coaUrl, } = data || {};
    if (!productSku || typeof productSku !== 'string' || !exports.CANONICAL_CATALOG_SKUS[productSku]) {
        throw new functions.https.HttpsError('invalid-argument', `Valid productSku from catalog is required. Allowed: ${Object.keys(exports.CANONICAL_CATALOG_SKUS).join(', ')}`);
    }
    const batchNumber = (rawBatchNumber || '').trim().toUpperCase();
    if (!batchNumber || batchNumber.length < 3) {
        throw new functions.https.HttpsError('invalid-argument', 'Valid batchNumber is required (minimum 3 characters).');
    }
    if (!manufactureDate || typeof manufactureDate !== 'string') {
        throw new functions.https.HttpsError('invalid-argument', 'Valid manufactureDate is required.');
    }
    if (!expiryDate || typeof expiryDate !== 'string') {
        throw new functions.https.HttpsError('invalid-argument', 'Valid expiryDate is required.');
    }
    ...
    if (!existingBatchQuery.empty) {
        throw new functions.https.HttpsError('already-exists', `Batch with batchNumber "${batchNumber}" already exists.`);
    }
    const batchRef = db.collection('batches').doc();
    const now = new Date().toISOString();
    const skuInfo = exports.CANONICAL_CATALOG_SKUS[productSku];
    const batchDoc = {
        id: batchRef.id,
        batchNumber,
        productSku,
 ...
 */
exports.updateBatchStatus = functions.https.onCall(async (data, context) => {
    const { callerUid, callerEmail, callerRoles } = await assertCanManageCodes(context);
    ...
    if (!status || !allowedStatuses.includes(status)) {
        throw new functions.https.HttpsError('invalid-argument', `Invalid status "${status}". Allowed: ${allowedStatuses.join(', ')}`);
    }
 ...
 */
async function checkUserSchoolAccess(uid, roles = [], email = '', isEmailVerified = false) {
 ...
 */
exports.getSchoolAccessStatus = functions.https.onCall(async (_data, context) => {
    if (!context.auth) {
 ...
 */
exports.createSchoolModule = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Caller must be authenticated.');
    }
    const callerUid = context.auth.uid;
 ...
 */
exports.updateSchoolModule = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Caller must be authenticated.');
    }
    const callerUid = context.auth.uid;
 ...
 */
exports.deleteSchoolModule = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Caller must be authenticated.');
    }
    const callerUid = context.auth.uid;
 ...
 */
exports.createSchoolLesson = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Caller must be authenticated.');
    }
    const callerUid = context.auth.uid;
    ...
    const { courseId, moduleId, title, contentMarkdown = { en: '' }, durationMinutes = 15, displayOrder = 0, isPublished = false, } = data;
 ...
 */
exports.updateSchoolLesson = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Caller must be authenticated.');
    }
    const callerUid = context.auth.uid;
 ...
 */
exports.deleteSchoolLesson = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Caller must be authenticated.');
    }
    const callerUid = context.auth.uid;
 ...
 */
exports.enrollInCourse = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
    ...
    if (!access.hasAccess) {
        throw new functions.https.HttpsError('permission-denied', 'School curriculum access requires 3 verified product container activations.');
    }
    const { courseId } = data;
    if (!courseId) {
        throw new functions.https.HttpsError('invalid-argument', 'Valid courseId is required.');
 ...
 */
exports.completeSchoolLesson = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Caller must be authenticated.');
    }
    const callerUid = context.auth.uid;
    ...
    const existingCompletedIds = progressSnap.exists
        ...
        : [];
    const completedSet = new Set(existingCompletedIds);
